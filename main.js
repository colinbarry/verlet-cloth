...
function makeModel(width, height) {
    ...
    for (let c = 0; c < width - 1; c++) {
      ...
      if (r < (height - 1)) {
        if ((r + c) % 2 === 1) {
          ...
          joins.push(makeJoin(pointByPosition(r, c + 1), pointByPosition(r + 1, c)))
        }
      }
    }
  }
  ...
  function tick(deltaTime) {
    ...
    const force = { x: Math.random() * 0.1, y: 0.3 }
    ...
    for (let particle of points) {
      ...
      particle.x = 2 * particle.x - particle.px + acceleration.x * (deltaTime * deltaTime)
      particle.y = 2 * particle.y - particle.py + acceleration.y * (deltaTime * deltaTime)
...
function distanceFromPointToLine(x0Line, y0Line, x1Line, y1Line, xPoint, yPoint) {
  const px = x1Line - x0Line
  const py = y1Line - y0Line
  const norm = px * px + py * py
  const u = clamp(((xPoint - x0Line) * px + (yPoint - y0Line) * py) / norm,
    0,
    1)
...
function makePointerHandler() {
  ...
  const handlePointerDown = appEl.addEventListener('pointerdown', e => {
    isDragging = true
  ...
  const handlePointerDrag = appEl.addEventListener("pointermove", e => {
    if (isDragging) {
      const pt = appEl.createSVGPoint()
      pt.x = e.clientX
      pt.y = e.clientY
      ...
      const points = model.points().filter(each => {
        return distanceFromPointToLine(prevPoint.x, prevPoint.y, txPoint.x, txPoint.y, each.x, each.y) < 0.03
      })
  ...
  const handlePointerUp = appEl.addEventListener("pointerup", e => {
    isDragging = false
  })
}
...
function render(app, points, joins) {
  const doc = `<g>`
    + joins.map(join => `<line x1="${join.from.x}" y1="${join.from.y}" x2="${join.to.x}" y2="${join.to.y}"></line>`).join('')
    + '</g>'
...
const pointerHandler = makePointerHandler()
...
document.body.addEventListener('click', e => {
  model.removePoint(model.points()[100])
})*/
